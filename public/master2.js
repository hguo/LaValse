    ...
    content: [
      {
        ...
        content: [{
          ...
          componentState: "machineView",
          width: 52
        }, {
          type: "component",
          componentName: "attributeView",
          componentState: "attributeView"
        }]
      },
      {
        type:'component',
        componentName: 'timelineView',
        componentState: "timelineView"
      }
    ]
  }]
};
...
layout.registerComponent("machineView", function(container, componentState) {
  $("#machineView").appendTo(container.getElement());

  container.on("resize", function() {
    var geom = container.getElement()[0].getBoundingClientRect();
    machineView.resize(geom); // TODO
  });
});

layout.registerComponent("attributeView", function(container, componentState) {
});

layout.registerComponent("timelineView", function(container, componentState) {
  $("#timeVolumeChart").appendTo(container.getElement());

  container.on("resize", function() {
    var geom = container.getElement()[0].getBoundingClientRect();
    timeVolumeChart.resize(geom);
  });
});
